...
// operator codes.
//
// Copyright (C) 1999 Paul Robert Hayes, Matt O'keefe
//
...
main(int argc, char **argv)
  ...
  fprintf(stdout, "\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "ToyPlaneFDTD\n");
  fprintf(stdout, "Copyright (C) 1998,1999 Paul Hayes, Matt O'Keefe\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "ToyPlaneFDTD is free software published under the terms\n"); 
...
//   dy = 0.01;
  ...
  for(i=0; i<=(nx+1); i++)
    {
    ez[i] = (double *)malloc(((ny)+2)*sizeof(double));
    for(j=0; j<=(ny+1); j++)
      {
      ez[i][j] = 0.0;
      }
  ...
  for(i=0; i<nx; i++)
    {
    hx[i] = (double *)malloc((ny+1)*sizeof(double));
    for(j=0; j<(ny+1); j++)
      {
      hx[i][j] = 0.0;
      }
  ...
  for(i=0; i<(nx+1); i++)
    {
    hy[i] = (double *)malloc((ny)*sizeof(double));
    for(j=0; j<ny; j++)
      {
      hy[i][j] = 0.0;
      }
  ...
  for(i=0; i<=(nx+1); i++)
    {
    hz[i] = (double *)malloc((ny+2)*sizeof(double));
    for(j=0; j<=(ny+1); j++)
      {
      hz[i][j] = 0.0;
      }
  ...
  for(i=0; i<=nx+1; i++)
    ...
    for(j=0; j<=ny+1; j++)
      ...
      deltaTimeEstimate = 1.0 / (velocity * sqrt( 1.0/(dx*dx) + 1.0/(dy*dy) ));
      //
